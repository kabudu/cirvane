...
Cirvane is private and under productisation. The enduring Recovery Scar identity is complete and source-controlled. The current firmware, USB prompt and application binary use the Cirvane name on ESP-IDF/FreeRTOS. Historical hardware evidence captured under the former Nucleus development identity is retained with that provenance. Kernel novelty research has a systematic prior-art matrix, a frozen recovery-transaction hypothesis, a portable kernel core with host tests, a two-slot configuration journal persisted in a reserved flash window, a fail-closed dual-slot rollback policy, a fail-closed UART/GPIO/timer/flash-read/erase/write/watchdog/entropy HAL, HIL and production spike compile profiles, and a quiet production kernel `cirvane>` shell with frozen crash and evidence lines. Radio on the kernel path, live `otadata` adapters, real-board adversarial qualification, matched evaluation, authenticated OTA transport and the initial developer release remain separately gated.

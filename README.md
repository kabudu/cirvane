...
Cirvane is private and under productisation. The enduring Recovery Scar identity is complete and source-controlled. The current firmware baseline is implemented and hardware-tested under its former Nucleus development identity. Kernel novelty research has a systematic prior-art matrix, a frozen recovery-transaction hypothesis, a portable kernel core with host tests, a two-slot configuration journal, a fail-closed dual-slot rollback policy, a fail-closed UART/GPIO/timer/flash-read/watchdog/entropy HAL, HIL and production spike compile profiles, and a quiet production `cirvane>` shell with frozen crash and evidence lines. Radio, durable flash adapters, real-board adversarial qualification, matched evaluation, service migration and rename, authenticated OTA transport and the initial developer release remain separately gated.

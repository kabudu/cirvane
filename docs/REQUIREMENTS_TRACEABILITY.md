...
| REL-04 | Production image owns scheduling without FreeRTOS runtime | Kernel and build | Link-map, symbol and runtime inspection | kernel-spike.json `freertos=absent` | Spike verified; production kernel is not a product image |
...
| REL-05 | Kernel HIL diagnostics compile-gated out of production profile | Spike build | Dual-profile ci-local strings/nm | HIL kernel-spike.json; production strings check | Compile profiles implemented; eval profile is the matched Cirvane image |

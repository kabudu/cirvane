...
| Kernel HAL | Cirvane kernel over C5 MMIO/ROM | Refuse malformed UART, GPIO, flash-read/erase/write, watchdog and entropy requests | Leave pin, flash and WDT state unchanged on refuse | GPIO pin 27, 256-byte flash copy, config-window erase/write, 8 MiB map |

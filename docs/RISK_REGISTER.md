...
| R13 | Security | Wi-Fi password leaks through shell history, logs, persistent storage or evidence | Low | High | Security owner | Secret scan, transcript or reboot inspection finds credential material | Mask separate password entry, keep ESP-IDF storage RAM-only, zero temporary buffers, clear on disconnect and exclude secrets from evidence |

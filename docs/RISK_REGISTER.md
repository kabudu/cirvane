...
| R9 | Architecture | Vendor Wi-Fi or radio components require FreeRTOS scheduling | High | Critical | Kernel owner | ESP-IDF Wi-Fi adapter on ESP32-C5 uses FreeRTOS tasks, queues and event groups | Isolate a genuinely non-kernel adapter, explicitly narrow the first release or stop the kernel release path. Owner decision is a Stage 2 gate |

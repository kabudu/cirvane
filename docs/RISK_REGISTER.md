...
| R9 | Architecture | Vendor Wi-Fi or radio components require FreeRTOS scheduling | High | Critical | Kernel owner | ESP-IDF Wi-Fi adapter on ESP32-C5 uses FreeRTOS tasks, queues and event groups | ADR 0005 keeps Wi-Fi on the ESP-IDF/FreeRTOS firmware. A kernel-labelled image still needs an isolated radio boundary, a no-Wi-Fi scope, or it remains research |

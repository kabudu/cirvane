...
- `ci.yml` runs the platform-neutral local gates in the pinned ESP-IDF v6.0.2 container. It proves brand exports repeat within the Linux renderer; the authoritative local gate additionally compares them with the curated macOS exports. It explicitly trusts only GitHub's runner-owned checkout path, has read-only repository permissions, a bounded timeout and no secrets.
